                ...
                <div class="panel-body">
                 <form name="dept" method="post" enctype="multipart/form-data">
                   <div class="form-group">
                    <label for="studentname">Student Name  <span style="color: red" id="errorMsg"></span></label>
  ...
  $(document).ready(function(){
    $('#studentname').on('blur',function(){
      if ($('#studentname').val().length <= 0) {
        $("#errorMsg").text('Enter a value!');
      }
      else {
       var regex = /^[a-zA-Z_ ]*$/;
       if (!regex.test($('#studentname').val())) {
        $('#studentname').focus();
        $("#errorMsg").text('Enter a valid student name!');
      }
      else {
        $("#errorMsg").text('');
